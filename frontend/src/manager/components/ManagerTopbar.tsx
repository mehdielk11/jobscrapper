...
export function ManagerTopbar({ orgName }: ManagerTopbarProps) {
  ...
  const initials = user?.email?.[0]?.toUpperCase() ?? 'M'
  ...
  return (
    <header className="h-14 flex-shrink-0 bg-background/80 backdrop-blur-md border-b border-border flex items-center justify-between px-6 sticky top-0 z-30 transition-colors duration-500">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-foreground font-['Sora',sans-serif]">{orgName}</span>
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          className="p-2 rounded-lg text-muted-foreground hover:text-emerald-500 hover:bg-muted transition-colors"
        >
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="w-7 h-7 rounded-full bg-emerald-500/20 flex items-center justify-center text-xs font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              {initials}
            </div>
